...
def price_colored_line(data): # make one with negative or positive
    ...
    data['color'] = ""
    data.loc[data['bin'] == 0, 'color'] = 'black'
    data.loc[data['bin'] == 1, 'color'] = 'green'
    data.loc[data['bin'] ==2, 'color'] = 'yellow'
    data.loc[data['bin'] == 3, 'color'] = 'orange'
    data.loc[data['bin'] == 4, 'color'] = 'red'


    x = data['Date']
    y = data['high']
    #c = data['bin'] #this color will do a full gradient
